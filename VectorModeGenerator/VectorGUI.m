...
%
% A GUI for splitted hologram manipulation with Matlab.
%
% To run the GUI just execute this script.
%
% The "user defined"  option accepts any matlab function (in 
...
assignin('base','sliderVal',sliderVal2);
set(handles.edit13,'String',num2str(sliderVal2));
updateAxes(handles);
...
set(handles.slider2,'Value',str2num(edit13));
updateAxes(handles);
...
phib=str2num(get(handles.edit15,'String'));
alf=cos(gamm);
bet=sin(gamm).*exp(1i*phib);

% Field for a Vortex beam LEFT side
if get(handles.radiobutton3,'Value')
        
    mL=str2num(get(handles.Topocharge,'String'));
    % r0=str2num(get(handles.Topocharge,'string'))/sqrt(abs(m)+1);
    r0L=str2num(get(handles.BeamWaist,'String'));
...
if get(handles.radiobutton10,'Value')
    % Spiral parameters
    mR=str2num(get(handles.edit11,'String'));
    % r0=str2num(get(handles.Topocharge,'string'))/sqrt(abs(m)+1);
